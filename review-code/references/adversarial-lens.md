...
- Idempotency & ordering: a webhook is replayed or delivered out of order; a job runs twice; a client retries after a partial success; two requests for the same operation interleave; a single-use token (coupon, gift card, reset or verification token) whose "already used" check and "mark used" write are non-atomic — two parallel redemptions both read unused and both commit; two related endpoints race (an email-change and a verification send) so the message goes to one address while the token binds to another — trace whether address selection and token binding read one consistent locked row.
- Input & validation bypass: malformed, oversized, deeply-nested, or unicode/encoding-tricked input slips past validation; a value valid at one layer is re-interpreted unsafely at the next; pagination/filter/sort params widen access to hidden rows; a request body mass-assigned into a persistence write — add `role`, `is_admin`, `plan`, `credits`, or `price` to the payload and trace whether it reaches the model without an explicit field allowlist; a negative, zero, or integer-overflowing quantity, or a currency field swapped while the numeric amount stays — trace whether the server recomputes the charge from a server-trusted price and currency rather than the client's line-item math.
...
- Concurrency on invariants: concurrent requests both pass a balance/quota/inventory check then both commit (double-spend); a limit enforced per-request but not across concurrent requests; a request reads or acts on an object still being built across multiple writes (password, owner, role, or status not yet set) — trace whether the row is visible before all invariant fields are committed.
...
- Trust boundary & privilege: a server-trusted internal call driven by user-controlled input (a confused deputy); an admin or service path reachable through a user-facing route; a signed token or capability reused beyond its intended scope, after expiry, or after it was consumed or superseded by a newer issuance (an old reset token still valid after a new one was requested or the password changed); a social/OAuth login or account-link accepted for an existing account solely because the provider-supplied email matches — trace whether it verifies email ownership or requires the existing account's credential.
