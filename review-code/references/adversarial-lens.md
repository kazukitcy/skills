...
- Idempotency & ordering: a webhook is replayed or delivered out of order; a job runs twice; a client retries after a partial success; two requests for the same operation interleave.
- Input & validation bypass: malformed, oversized, deeply-nested, or unicode/encoding-tricked input slips past validation; a value valid at one layer is re-interpreted unsafely at the next; pagination/filter/sort params widen access to hidden rows.
