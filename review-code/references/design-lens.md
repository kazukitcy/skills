...
- Responsibility & layering: business logic placed in a controller/view/handler instead of the domain layer; a UI or transport layer reaching into data-layer internals; a dependency pointing the wrong direction; a new import that creates a cycle.
...
- Backward compatibility: a change that silently alters the meaning of an existing field or response; a serialized/persisted shape changed without a versioning or compatibility story.
- Coupling & abstraction: tight coupling introduced (a module that now must change in lockstep with another); a leaky abstraction that hides an important failure mode; premature generalization with a single caller that obscures behavior; an invariant left unencapsulated where any caller can violate it.
