...
- Authentication: a route/handler/server-action reachable without a server-side identity check; auth enforced only in the client or by a hidden UI; a token accepted without signature/expiry/audience verification; JWT verification that lets the token's own `alg` header pick the algorithm (no explicit `algorithms` allowlist — RS256→HS256 downgrade, `alg:none`), that selects the verification key from the token's own header (`jku`/`x5u` fetched without a host allowlist, `kid` used unsanitized in a file path or query), that calls a decode-only API where verify is intended, or that skips `iss`/`aud` comparison so a token minted for another service or tenant is accepted; a password-reset, verification, or magic-link URL built from the request's `Host`/`X-Forwarded-Host` header instead of a configured base URL (reset-token capture via a spoofed host).
- OAuth/OIDC flows: a callback handler that exchanges the authorization `code` without validating `state` (or PKCE/`nonce`) against the value bound to the initiating session (login CSRF — skip when the configured OAuth client library demonstrably performs `state` validation itself); a post-login or logout redirect target (`next`, `returnTo`, `redirect_uri`) taken from the request and followed without an allowlist or relative-path check (open redirect that hands off a fresh session or leaks the OAuth code via `Referer`; a redirect validated same-origin or path-only is fine).
- Authorization & ownership: object access by user-supplied ID without an ownership or tenant check (IDOR); a role/permission check missing on a state-changing path; authorization decided before, but not re-checked after, a redirect or indirect call (confused deputy); a GraphQL field or nested-object resolver returning protected data with authorization checked only at the query root (skip when a schema-wide authz directive, plugin, or middleware demonstrably covers the type); a caught exception or error branch inside an auth, permission, or verification check that falls through to allow (fail-open — a `catch` that returns success, or a verification error logged while execution continues).
...
- Input contracts: mass assignment of request fields into a persistence model; file uploads without content-type/extension validation, or stored where they can be executed or rendered (stored XSS, webshell); unsafe deserialization of untrusted bytes; a deep merge or recursive property copy of user-controlled objects without `__proto__`/`constructor` key filtering (prototype pollution — JS runtimes only).
...
- Web/transport hardening: missing CSRF protection on a cookie-authenticated state-changing route; overly broad CORS combined with credentials; cookies missing `HttpOnly`/`Secure`/`SameSite`; missing webhook signature or replay (timestamp/nonce) verification; a bearer token, session ID, or OAuth code passed or returned in a URL query parameter, leaking into access logs, browser history, and `Referer` headers; an HMAC, webhook signature, or token digest compared with `==`/`===` instead of a constant-time compare (report only when the compared value is attacker-supplied and the endpoint's response observable).
- Surface & dependency changes: a new dependency, scope, or permission that widens the attack surface; a previously internal endpoint newly exposed; auth or crypto downgraded (weaker algorithm, disabled verification); an LLM agent's tool definitions granting write-, send-, or exec-scope beyond what its task needs; an IaC or policy diff (Terraform, CloudFormation, IAM JSON) that widens access — a wildcard `Action`/`Resource`, security-group ingress from `0.0.0.0/0` to an admin or database port, a disabled public-access block or encryption setting (skip intentionally public resources, e.g. a static-site bucket or 443 on a public load balancer).
