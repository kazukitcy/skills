# Microsoft Rust Guidelines: Libraries: UX

Source: <https://github.com/microsoft/rust-guidelines> at commit `95ac4c828fb8`, `src/guidelines/libs/ux/`. Copyright (c) Microsoft Corporation, MIT license (see `LICENSE-MS-GUIDELINES` at the skill root). Text lightly reformatted: anchors and cross-links converted, figures omitted.

<a id="m-simple-abstractions"></a>
## Abstractions don't visibly nest (M-SIMPLE-ABSTRACTIONS)
**Why:** low cognitive load and a good out-of-the-box UX.
...
**Why:** low cognitive load and ergonomic APIs.
...
**Why:** composable patterns and freedom from design lock-in.
...
[M-MOCKABLE-SYSCALLS]: ms-libs-resilience.md#m-mockable-syscalls
[M-ABSTRACTIONS-DONT-NEST]: #m-simple-abstractions

<a id="m-errors-canonical-structs"></a>
## Errors are canonical structs (M-ERRORS-CANONICAL-STRUCTS)
**Why:** harmonized error types and consistent error handling.
...
**Why:** future-proof type construction in complex scenarios.
...
**Why:** construction free of parameter mix-ups.
...
**Why:** composable sharing of common services.
...
**Why:** easily discoverable essential functionality.
...
**Why:** discoverable functionality and clear API usage.
...
**Why:** a clean namespace and reliable downstream builds.
...
**Why:** simpler code and easier-to-understand APIs.

Functions should be declared `async fn foo()` over `fn foo() -> impl Future` when both are viable.

Functions marked `async` are more idiomatic and easier to read. An explicit `Future`-returning signature should only be used when required, for example inside traits or for _hot 'n heavy_ async functions, compare [M-ASYNC-STACK-SIZE](ms-performance.md#m-async-stack-size).
